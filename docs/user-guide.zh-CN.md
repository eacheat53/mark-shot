...
独立 OCR 结果窗口默认显示可编辑的识别原文。点击“翻译”后，窗口展开译文区和
目标语言控件，并使用已保存的语言开始翻译。两边均可单独复制，复制反馈显示在
对应按钮上，编辑后会出现撤销入口。宽窗口并排显示原文和译文，窄窗口上下排列；
拖动分隔条可以调整两边的空间，右下角可以调整窗口大小。通过“更多操作 → 原图”
查看本次识别的截图缩略图。
...
视频质量，展开后可以调整详细参数、倒计时和采集后端。切换视频与 GIF 会保留
...
从托盘菜单打开“设置”。窄窗口使用顶部分类选择器；高级分组在当前页面展开，收起
