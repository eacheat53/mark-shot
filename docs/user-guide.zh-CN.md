...
独立 OCR 结果窗口保留可编辑的识别原文，翻译时另行展开译文区，两边均可单独复制。
宽窗口并排显示原文和译文，窄窗口上下排列；拖动分隔条可以调整两边的空间，
右下角可以调整窗口大小。点击“原图”可展开本次识别的截图缩略图。
目标语言和翻译按钮位于底部，翻译期间可以取消；原文或目标语言改变后，
窗口会提示重新翻译。`Ctrl+Enter` 翻译，`Ctrl+Shift+C` 复制当前编辑区全文，
